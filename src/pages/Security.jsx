  ...
  return (
    ...
    >
      ...
      <main className="flex-1 w-full mx-0 px-4 sm:px-6 md:px-8 lg:px-10 xl:px-14 py-5 relative">
        ...
        {/* CTA - neutral, no solid black box */}
        <div className={`mt-6 border ${bx} p-2`}>
          <div
            className={`p-6 sm:p-8 text-center border ${bx}`}
            style={{ background: "transparent" }}
          >
            <a
              href="/contact"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-md border border-current/60 transition hover:bg-black/5 dark:hover:bg-white/10"
...
/**
 * Responsive RailItem
 * - No fixed min width; never overflows
 * - On mobile: hint shows under label (no arrow UI)
 * - On sm+: arrow to hint swap on hover; right column clamps and truncates
 */
function RailItem({ label, hint, orange = false }) {
  ...
  return (
    <div
      className={`group relative flex items-start justify-between gap-2 py-2 sm:py-2.5 border-b ${bx} cursor-default`}
    >
      {orange && (
        <span
      ...
      {/* Left: label + (mobile hint) */}
      <div className="pl-2 text-sm sm:text-base leading-snug">
        <span className="block">{label}</span>
        <span className={`${mute} block text-[11px] sm:hidden mt-0.5`}>
          {hint}
        </span>
      </div>

      {/* Right: arrow/hint swap (hidden on mobile) */}
      <div className="relative hidden sm:block w-40 md:w-44 lg:w-48 text-right overflow-hidden">
        {/* default view: arrow */}
        <span
          className={`${mute} text-xs block transition-opacity duration-200 group-hover:opacity-0`}
        >
          <span className="inline-flex w-full justify-end">→</span>
        </span>

        {/* hover view: hint (truncated to avoid spill) */}
        <span className="absolute inset-0 right-0 text-[11px] opacity-0 transition-opacity duration-200 group-hover:opacity-100 truncate">
          {hint}
        </span>
      </div>
    </div>
...
/* One-shot scramble on first mount (no em dash in charset) */
function ScrambleTextOnce({
  ...
  React.useEffect(() => {
    ...
    // Seed view with scrambled string first
    const seed = scrambleSeed(text, chaos);
    setOut(seed);
